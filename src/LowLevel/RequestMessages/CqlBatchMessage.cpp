...
namespace cql {
	...
	void CqlBatchMessage::encodeBody(const CqlConnectionInfo&, seastar::sstring& data) const {
		...
		CqlProtocolShort queryCount(queryCountValue);
		if (queryCount.get() != queryCountValue) {
			throw CqlLogicException(CQL_CODEINFO, "too many queries");
		}
		queryCount.encode(data);
		// <query_i>
		CqlProtocolByte kind;
		CqlProtocolInt stringSize; // query string size
		CqlProtocolShort idSize; // prepared query id size
		...
		for (std::size_t i = 0, j = batchCommand.getQueryCount(); i < j; ++i) {
			...
			if (preparedQueryId.empty()) {
				kind.set(enumValue(CqlBatchQueryKind::Query));
				stringOrId = batchCommand.getQuery(i);
				stringSize.set(stringOrId.second);
			} else {
				kind.set(enumValue(CqlBatchQueryKind::PreparedQueryId));
		...
		if (queryCountValue != queryCountVerify) {
			throw CqlLogicException(CQL_CODEINFO, "incorrect query count calculation");
		}
		// <batch_parameters>
	...
	const seastar::sstring& CqlBatchMessage::getPreparedQueryId(std::size_t index) const& {
	...
	/** Get the prepared query id of the query at the specificed index */
	seastar::sstring& CqlBatchMessage::getPreparedQueryId(std::size_t index) & {
		if (index < preparedQueryIds_.size()) {
			return preparedQueryIds_[index];
		} else {
			auto& batchCommand = batchParameters_.getBatchCommand();
