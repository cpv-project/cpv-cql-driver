...
namespace cql {
	...
	public:
		...
		const CqlProtocolColumnOption& getType() const& { return type_; }
		CqlProtocolColumnOption& getType() & { return type_; }
		void setType(const CqlProtocolColumnOption& type) { type_ = type; }
		void setType(CqlProtocolColumnOption&& type) { type_ = std::move(type); }
		...
		/** Constructor */
		CqlProtocolResultColumn();

	private:
		CqlProtocolString keySpace_;
		CqlProtocolString table_;
		CqlProtocolString name_;
		CqlProtocolColumnOption type_;
	};
}
