...
namespace cql {
	...
	void ProtocolResultRowsMetadata::setPagingState(const std::string& pagingState) {
		pagingState_.set(pagingState);
		flags_.set(enumValue(getFlags() | ResultRowsMetadataFlags::HasMorePages));
	}

	/** Set the value used to retrieve the next page of results, also indicates there are more pages */
	void ProtocolResultRowsMetadata::setPagingState(std::string&& pagingState) {
	...
	void ProtocolResultRowsMetadata::encode(std::string& data) const {
		auto flags = getFlags();
		flags_.encode(data);
