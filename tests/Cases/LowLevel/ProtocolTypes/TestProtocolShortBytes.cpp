...
#include <LowLevel/ProtocolTypes/ProtocolShortBytes.hpp>
#include <TestUtility/GTestUtils.hpp>

TEST(TestProtocolShortBytes, getset) {
	cql::ProtocolShortBytes value("abc");
	ASSERT_EQ(value.get(), "abc");
	value.get().append("aaaaa", 5);
	ASSERT_EQ(value.get(), "abcaaaaa");

	value = cql::ProtocolShortBytes("");
	ASSERT_EQ(value.get(), "");
}

TEST(TestProtocolShortBytes, encode) {
	cql::ProtocolShortBytes value("abc");
	std::string data;
	value.encode(data);
...
TEST(TestProtocolShortBytes, decode) {
	cql::ProtocolShortBytes value("");
	{
...
TEST(TestProtocolShortBytes, decodeError) {
	{
		cql::ProtocolShortBytes value("");
