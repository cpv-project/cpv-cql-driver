...
namespace cql {
	 ...
	 */
	class CqlTimeStamp : private CqlDateTimeBase {
	public:
		...
		/** Constructors */
		CqlTimeStamp() : CqlDateTimeBase() { }
		template <class... Args>
		CqlTimeStamp(Args&&... args) : CqlDateTimeBase() {
		...
		/** Create a CqlTimeStamp with specificed year-month-day hour:minute:second */
		static CqlTimeStamp create(
			...
			return CqlTimeStamp(localTm);
		}

		/** Create a CqlTime with now, in local timezone */
		static CqlTimeStamp now() {
			return CqlTimeStamp(std::chrono::system_clock::now());
		}
	};

	/** Write text description expressed in local timezone of timestamp */
	std::ostream& operator<<(std::ostream& stream, const CqlTimeStamp& time) {
		std::tm localTm = time;
		...
		stream << std::setfill('0') <<
			...
			std::setw(3) << (milliSeconds.count() % CqlTimeStamp::MilliSecondsPerSecond);
		return stream;
	}
}
